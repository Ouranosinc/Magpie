...
mandatory parts are the ``MAGPIE_COOKIE_NAME`` value, the actual token value, and the indication ``!userid_type:int``
that tells `Magpie` the provided token information is employed to resolve the :term:`Logged User` by ID.
