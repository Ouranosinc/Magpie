...
from beaker.cache import Cache, cache_regions
...
from magpie.services import SERVICE_TYPE_DICT, ServiceInterface, service_factory
...
def get_service_or_resource_types(service_or_resource):
    ...
    """
    svc_res_type_cls = None
    svc_res_type_str = None
...
def invalidate_acl(resource, db_session, user=None, group=None):
    # type: (ServiceOrResourceType, Session, Optional[models.User], Optional[models.Group]) -> None
    """
    Invalidates any cached :term:`ACL` reference matching specified :term:`User` and :term:`Resource` combination(s).

    Although actual :term:`ACL` cache definitions refer to a combination
    of ``(service, request_method, request_path_qs, user_id)`` that should be invalidated on :term:`Permission` change,
    reverse engineering of all applicable :term:`Permission` and their conversion from the many different service-type
    parsing implementations, HTTP method and path/query-string combinations make it more complicated and intensive
    code/compute-wise than simply invalidating all matching ACL for this user/resource, regardless of how it is
    accessed by them permission-wise. The cache will be re-applied correctly from scratch on any following request
    matching those user/resource combinations, making the impact of flushing all their :term:`ACL` relatively minimal
    since :term:`Permission` should not change as often as effective access requests.

    :param resource: corresponding service or resource under a service for which to invalidate user(s) ACL.
    :param db_session: database connection to retrieve the resource's service.
    :param user: user for which to invalidate ACL (skip if `None`).
    :param group: if provided, applies the ACL invalidation operation for every user member of that group.
    """
    # skip if caching not applicable
    if not cache_regions.get("acl", {}).get("enabled", False):
        return
    users = ([user] if user else []) + (group.users if group else [])
    if not users:
        return
    svc = get_resource_root_service(resource, db_session)
    ...
    for namespace in [ServiceInterface._get_acl_cached]:
        ...
        # only about service-name and user-id matter in this case
        # ACL full signature: (service_name, request_method, request_path, user_id)
        svc_param_key = (svc_name + " ").encode("ascii", "backslashreplace")
        for func_params_key in list(cache.namespace.dictionary):
            for usr in users:
                usr_id_param_key = (" " + str(usr.id)).encode("ascii", "backslashreplace")
                if func_params_key.startswith(svc_param_key) and func_params_key.endswith(usr_id_param_key):
                    LOGGER.info("Invalidating ACL for user [%s, %s] and service [%s]", usr.id, usr.user_name, svc_name)
