...
from magpie.api import api_rest_schemas as s, api_except as ax
...
def get_value_multiformat_post_checked(request, key, default=None):
    ...
    ax.verify_param(val, notNone=True, notEmpty=True, httpError=HTTPUnprocessableEntity,
...
def get_user(request, user_name_or_token):
    if user_name_or_token == get_constant('MAGPIE_LOGGED_USER'):
        ...
        if curr_user:
            ...
            anonymous = ax.evaluate_call(lambda: zig.UserService.by_user_name(anonymous_user, db_session=request.db),
                                         fallback=lambda: request.db.rollback(), httpError=HTTPForbidden,
                                         msgOnFail=s.User_CheckAnonymous_ForbiddenResponseSchema.description)
            ax.verify_param(anonymous, notNone=True, httpError=HTTPNotFound,
        ...
        if admin_principal not in principals:
            raise HTTPForbidden()
        user = ax.evaluate_call(lambda: zig.UserService.by_user_name(user_name_or_token, db_session=request.db),
...
def get_group_matchdict_checked(request, group_name_key='group_name'):
    ...
    group = ax.evaluate_call(lambda: zig.GroupService.by_group_name(group_name, db_session=request.db),
                             fallback=lambda: request.db.rollback(), httpError=HTTPForbidden,
...
def get_resource_matchdict_checked(request, resource_name_key='resource_id'):
    ...
    resource_id = ax.evaluate_call(lambda: int(resource_id), httpError=HTTPNotAcceptable,
                                   msgOnFail=s.Resource_MatchDictCheck_NotAcceptableResponseSchema.description)
    resource = ax.evaluate_call(lambda: zig.ResourceService.by_resource_id(resource_id, db_session=request.db),
                                fallback=lambda: request.db.rollback(), httpError=HTTPForbidden,
                                msgOnFail=s.Resource_MatchDictCheck_ForbiddenResponseSchema.description)
    ax.verify_param(resource, notNone=True, httpError=HTTPNotFound,
...
def get_service_matchdict_checked(request, service_name_key='service_name'):
    ...
    ax.verify_param(service, notNone=True, httpError=HTTPNotFound, content={u'service_name': service_name},
