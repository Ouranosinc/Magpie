...
def get_users_groups(db_session):
    ...
    """
    all_users = db_session.query(UserMixin)
    all_groups = db_session.query(GroupMixin)
...
def upgrade_migrate(old_group, old_user, new_group, new_name, db_session):
    ...
    if old_user is not None:
        for usr_grp in db_session.query(UserGroupMixin):
...
def downgrade_migrate(old_group, old_user, new_group, old_name, db_session):
    ...
    if old_group is None:
        ...
        old_group = GroupMixin(group_name=old_name)   # noqa
    ...
    if old_user is None:
        ...
        old_user = UserMixin(user_name=old_name, email=email)  # noqa
        db_session.add(old_user)
        old_user = UserService.by_user_name(old_name, db_session)
        usr_grp = UserGroupMixin(group_id=old_group.id, user_id=old_user.id)  # noqa
